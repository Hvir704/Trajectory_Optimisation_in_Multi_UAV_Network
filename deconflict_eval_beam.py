...
def eval_seed_cell(pol, M, K, Emax_each, inst_seeds, delta, dt, device, beam_width):
    ...
    with torch.no_grad():
        for s in inst_seeds:
            env = Env(M=M, seed=s)
            f = fleet_rollout_beam(pol, env, K, device, Emax_each=Emax_each,
...
def main():
    ...
    ap.add_argument('--out-dir',   default='./deconflict_beam_out')
    ...
    ap.add_argument('--beam-width', type=int, default=5,
                    help='beam width for the routing rollout (matches beam_eval)')
        ...
        for M in args.M:
            for K in args.K:
                ...
                for seed in args.seeds:
                    ...
                    r = eval_seed_cell(pol, M, K, Ee, inst_seeds, args.delta, args.dt,
                                       device, args.beam_width)
    ...
    with open(tbl, 'w') as f:
        f.write("POST-HOC TEMPORAL DECONFLICTION on BEAM-AUGMENTED routing — final collision-avoided objective\n")
        f.write(f"delta={args.delta}m, common-t=0 AoI penalty = theta1*sum(o_k*W_total_k), "
                f"theta1={P.theta1}\n")
        f.write("routing = BEAM+pp fleet_objective (compare vs greedy deconflict_table.txt, SAME instances).  "
                "FINAL = routing + deconfliction penalty.\n")
        f.write("lower = better.  +/- = std over seeds.\n")
        f.write("=" * 104 + "\n")
        f.write(f"{'M':>4}{'K':>3}  {'routing(BEAM+pp)':>20}{'penalty':>14}{'pen%':>7}"
